...
import com.mdasrafulalam.cricwave.databinding.FragmentPlayersDetailsT20Binding
...
class PlayersDetailsT20Fragment : Fragment() {
    private lateinit var binding: FragmentPlayersDetailsT20Binding
    ...
    override fun onCreateView(
        ...
        binding = FragmentPlayersDetailsT20Binding.inflate(layoutInflater,container,false)
        return binding.root
    }

    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        //count all batting scores
        lifecycleScope.launch {
            val playersCareer = viewModel.getPlayerByIdIncludingCareer(MyConstants.CURRENT_PLAYER_ID)
            val batting = playersCareer.filter { it.type == "T20" || it.type=="T20I" }.map { it.batting }
            val bowling = playersCareer.filter { it.type == "T20" || it.type=="T20I"  }.map { it.bowling }
            //battting
            ...
            binding.t20BattingData = odiBatting
            ...
            binding.t20BowlingData = odiBowling
        }
        binding.playersT20BatttingShowTV.setOnClickListener {
            if (!isBattingShown){
                isBattingShown = true
                binding.battingLayout.visibility = View.VISIBLE
                binding.playersT20BatttingShowTV.setCompoundDrawablesWithIntrinsicBounds(0,0,R.drawable.collaps,0)
            }else{
                isBattingShown = false
                binding.battingLayout.visibility = View.GONE
                binding.playersT20BatttingShowTV.setCompoundDrawablesWithIntrinsicBounds(0,0,R.drawable.expand,0)
            }
        }
        binding.playersT20BowlingShowTV.setOnClickListener {
            if (!isBowlingShown){
                isBowlingShown = true
                binding.bowlingLayout.visibility = View.VISIBLE
                binding.playersT20BowlingShowTV.setCompoundDrawablesWithIntrinsicBounds(0,0,R.drawable.collaps,0)
            }else{
                isBowlingShown = false
                binding.bowlingLayout.visibility = View.GONE
                binding.playersT20BowlingShowTV.setCompoundDrawablesWithIntrinsicBounds(0,0,R.drawable.expand,0)
            }
        }
    }
