...
import com.mdasrafulalam.cricwave.databinding.FragmentPlayersDetailsOdiBinding
...
class PlayersDetailsOdiFragment : Fragment() {
    private lateinit var binding: FragmentPlayersDetailsOdiBinding
    ...
    override fun onCreateView(
        ...
        binding = FragmentPlayersDetailsOdiBinding.inflate(layoutInflater, container, false)
        return binding.root
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        lifecycleScope.launch {
            val playersCareer =
                viewModel.getPlayerByIdIncludingCareer(MyConstants.CURRENT_PLAYER_ID)
            val batting = playersCareer.filter { it.type == "ODI" }.map { it.batting }
            val bowling = playersCareer.filter { it.type == "ODI" }.map { it.bowling }
            //battting
            ...
            val odiBatting = Batting(
                round(totalAveragesBatting),
                0,
                totalfifties,
                totalfours,
                0.0,
            ...
            binding.odiBattingData = odiBatting
            ...
            val odiBowling = Bowling(
                round(totalAveragesBowling),
                round(totalEconRate),
                totalfivewickets,
                0,
                totalinningsbowling,
            ...
            binding.odiBowlingData = odiBowling
        }
        binding.playersOdiBatttingShowTV.setOnClickListener {
            if (!isBattingShown) {
                isBattingShown = true
                binding.battingLayout.visibility = View.VISIBLE
                binding.playersOdiBatttingShowTV.setCompoundDrawablesWithIntrinsicBounds(
                    0,
                    0,
                    R.drawable.collaps,
                    0
        ...
        binding.playersOdiBowlingShowTV.setOnClickListener {
            if (!isBowlingShown) {
                isBowlingShown = true
                binding.bowlingLayout.visibility = View.VISIBLE
                binding.playersOdiBowlingShowTV.setCompoundDrawablesWithIntrinsicBounds(
                    0,
                    0,
                    R.drawable.collaps,
                    0
